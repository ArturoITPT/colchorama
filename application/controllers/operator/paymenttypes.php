...
class Paymenttypes extends super_controller{
    
    private $view = "list";
    private $id = null;
    private $dir = 'operator';
    private $controller='paymenttypes';
    private $Model = 'Paymenttype';
    protected $title = "Tipos de Pago";
    private $add_label = "Agregar Tipo de Pago";
    ...
    function formContent(){
        ...
        $label = "Tipo";
        $options=array();
        $options["Efectivo"]="Efectivo";
        $options["Cheque"]="Cheque";
        $options["Tarjeta Bancaria"]="Tarjeta Bancaria";
        $options["Crédito"]="Crédito";
        $options["Apartado"]="Apartado";
       
        $input = new Multiselect("type",$options, array(0=>$record->type));
        $input->setAttr("required","required");
        $input->setClass("form-control");
        $element = $this->FormElement($label,$input);     
        $form->add($element);
