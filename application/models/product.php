 ...
 */
class Product extends parent_model {
	...
	var $has_one = array('provider','category','brand');

	// Insert related models that Template can have more than one of.
	var $has_many = array('image','order','receipt');
...
//                ////
   ...
   function selectUnitsOfMeasure(){
       
       $u = new Unitsofmeasure();
       $all = $u->get()->all;
       $all = parent_model::allToOptions2($all, $this->unitsofmeasure_id);
       return $all;
       
   }
    function displayOptions($Model,$field){
       
       $u = new $Model();
       $all = $u->get()->all;
       $all = parent_model::allToOptions2($all, $this->$field);
       return $all;
       
   }
   
   
   function getImage($string =false,$dim = 100 ){
       
       $image = $this->image->get(1);
       
       $img = new Img(base_url("uploads/{$image->raw_name}_thumb{$image->file_ext}"));
       $img->setDim($dim);
       if ($string)
           return "$img";
       else
   ...
   function getImageSrcThumb( ){
       
       $image = $this->image->get(1);
       
       $img = (base_url("uploads/{$image->raw_name}_thumb{$image->file_ext}"));
       
       return $img;
       
   }
   function save($object= null,$related_field =null){
       //echo "saving child ....";
       //echo "child value ";2334066
   ...
   function getJoinTotal(){
       
       $receipt = $this->receipt->include_join_fields()->get();
       
       $qty = $receipt->join_qty;
       $cost = $receipt->join_cost;
